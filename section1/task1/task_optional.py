...
class CryptoCompareAPI():
    ...
    def get_toplist(self, tsym, limit=None, page=None):
        tsym = tsym.upper()
        if limit != None and page != None:
            while True:
              limit = limit
              if limit < 10 or limit > 100:
                  print("Limit is between 10 and 100, please try one more time.")
                  break
              else:
                  top_url = "data/top/mktcapfull?limit={}&page={}&tsym={}".format(limit, page, tsym)
            return pd.DataFrame(self._safeRequest(self.url + top_url))
        elif limit != None and page == None:
            while True:
              limit = limit
              if limit < 10 or limit > 100:
                  print("Limit is between 10 and 100, please try one more time.")
                  ...
                  top_url = "data/top/mktcapfull?limit={}&page={}&tsym={}".format(limit, tsym)
            return pd.DataFrame(self._safeRequest(self.url + top_url))
        elif limit == None and page != None:
            top_url = "data/top/mktcapfull?tsym={}&page={}".format(tsym, page)
            return pd.DataFrame(self._safeRequest(self.url + top_url))
        elif limit == None and page == None:
            top_url = "data/top/mktcapfull?tsym={}".format(tsym)
            return pd.DataFrame(self._safeRequest(self.url + top_url))
        else:
            raise ValueError(f"Can't do it")
...
df2 = cc_api.get_toplist('USDT')
df2.to_csv(os.path.join(DATA_DIR, "Top_list.csv"), index=False)